...
> **Workflow 1.5 (`/experiment-bridge`) 的输入模板。** 填写后保存为 `refine-logs/EXPERIMENT_PLAN.md`。
>
> FarField 编译后的实验设计（`EXPERIMENT_PLAN.md` 与 `EXPERIMENT_PLAN_EN.md`）是自包含的：主张、两臂、实验块、运行顺序、预算和风险都写在本文件。同一套实验块也会编进研究方案，不必再跳文件。
